...
export function TransactionForm({ 
  ...
  const [paymentMode, setPaymentMode] = useState(initialData?.paymentMode || 'upi');
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-lg max-h-[90vh] p-0">
        <DialogHeader className="px-6 pt-6">
          ...
          <DialogDescription>
            {initialData ? 'Update transaction details' : 'Record a new transaction'}
          </DialogDescription>
        </DialogHeader>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-2">
                    {billImages.map((url, index) => (
                      <div key={index} className="relative aspect-square rounded-lg overflow-hidden border bg-muted ring-2 ring-primary/50">
                        <img
                          src={url}
