...
export async function GET() {
  try {
    ...
    const [supplierCountResult, customerCountResult, supplierPendingResult, customerPendingResult] =
      await Promise.all([
        // Count suppliers
        ...
        supabase
          .from("udhar")
          .select(
            "amount, cash_amount, online_amount, paid_amount, paid_cash, paid_online, payment_status"
          )
          .neq("payment_status", "paid"),
      ]);
    ...
    const totalCustomerPending = (customerPendingResult.data || []).reduce((sum, u) => {
      const totalAmount =
        Number(u.amount) || (Number(u.cash_amount) || 0) + (Number(u.online_amount) || 0);
      const paidAmount =
        Number(u.paid_amount) || (Number(u.paid_cash) || 0) + (Number(u.paid_online) || 0);
      return sum + Math.max(0, totalAmount - paidAmount);
    }, 0);
