...
export default function ReportsPage() {
  ...
  const paymentStatusData = useMemo(() => {
    const paid = filteredTransactions.filter(t => t.paymentStatus === 'paid').length;
    const pending = filteredTransactions.filter(t => t.paymentStatus === 'pending').length;
    const partial = filteredTransactions.filter(t => t.paymentStatus === 'partial').length;
  ...
  const stats = useMemo(() => {
    ...
    const prevStart = subMonths(dateRange.start, 
      timeRange === '1month' ? 1 : 
      timeRange === '3months' ? 3 : 
    ...
    const changePercent = prevTotal ? ((totalAmount - prevTotal) / prevTotal * 100).toFixed(1) : 0;
    ...
    return {
      ...
      avgTransaction: filteredTransactions.length ? totalAmount / filteredTransactions.length : 0
    };
        ...
        <Card className="card-lift">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              ...
              <div className={`flex items-center gap-1 text-sm ${stats.changePercent >= 0 ? 'text-green-500' : 'text-red-500'}`}>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-500">₹{stats.paidAmount.toLocaleString()}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">₹{Math.round(stats.avgTransaction).toLocaleString()}</p>
              </div>
